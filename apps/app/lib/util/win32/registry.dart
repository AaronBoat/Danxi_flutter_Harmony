...
class Registry {
  ...
  static int getRegistryKeyHandle(int hive, String key) {
    ...
    try {
      if (RegOpenKeyEx(hive, lpKeyPath, 0, REG_SAM_FLAGS.KEY_READ, phKey) != WIN32_ERROR.ERROR_SUCCESS) {
  ...
  static RegistryKeyValuePair? enumerateKey(int hKey, int index) {
    ...
    try {
      ...
      switch (status) {
        case WIN32_ERROR.ERROR_SUCCESS:
          if (lpType.value != REG_VALUE_TYPE.REG_SZ) throw Exception('Non-string content.');
  ...
  static void setStringValue(int hKey, String keyName, String value) {
    ...
    int status = RegSetValueEx(hKey, keyName.toNativeUtf16(), 0, REG_VALUE_TYPE.REG_SZ,
